...
For consistency and to ease readability, contracts should layout sections in the following order. In each specific section, entries should be sorted alphabetically.
...
1. Imports
   ...
   1. Interface imports
1. Custom errors
1. Type `using` clauses.
1. Structs
1. Data
   1. Mutable data fields (may not be sorted for upgrade compatibility reasons)
   1. Immutable data fields
   1. Constants
1. Events
1. Modifiers
1. Constructor
...
1. Fallback function
1. Functions
   1. Public/External **write** functions
   1. Internal/Private **write** functions (prefixed with an underscore)
   1. Public/External **view** functions
   1. Internal/Private **view** functions (prefixed with an underscore)
1. Storage gap
...
We use [Slither](https://github.com/crytic/slither) which is a Solidity static analysis framework written in Python 3. It runs a suite of vulnerability detectors, prints visual information about contract details. To run slither:

```
virtualenv venv
source venv/bin/activate
pip3 install slither-analyzer
cd fnd-contracts/ && slither contracts --solc-remaps @=node_modules/@
```

Note: If Slither complains about the version of Solidty thats being used, you can:

```
pip3 install solc-select
solc-select install X.Y.Z (Version)
